 ...
 */
export function setupForm(formId: string, onSubmit: FormSubmitHandler, onSuccess?: SuccessCallback): void {
 ...
 */
function setLoading(button: HTMLButtonElement | null, loading: boolean, originalText?: string): void {
    if (!button) return;
...
function showFormError(form: HTMLFormElement, message: string): void {
    const errorEl = form.querySelector('[data-field-error]') as HTMLElement | null;
    if (errorEl) {
 ...
 */
async function handleSubmit(form: HTMLFormElement, onSubmit: FormSubmitHandler, onSuccess?: SuccessCallback): Promise<void> {
