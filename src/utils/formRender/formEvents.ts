...
export function createFormEvents(
	...
	function handlePasswordVisibility(button: HTMLElement): (e: Event) => void {
		return (e: Event): void => {
			...
			const confirmInput = wrapper?.querySelector(
				'input[name="passwordConfirm"]',
			) as HTMLInputElement | null;
			const targetInput = input || confirmInput;
			if (targetInput) {
				actions.togglePassword(
					targetInput.name as 'password' | 'passwordConfirm',
				);
			}
		};
	}
	...
	function redrawPasswordValidator(input: HTMLInputElement): void {
		...
		const template =
			typeof partial === 'function'
				? partial
				: Handlebars.compile(partial as string);
	...
	function attach(): void {
		...
		regPasses?.forEach((input) => {
			...
			passwordInput.addEventListener(
				'focus',
				showPasswordValidator(passwordInput),
			);
			passwordInput.addEventListener(
				'blur',
				hidePasswordValidator(passwordInput),
			);
			passwordInput.addEventListener(
				'input',
				handlePasswordValidator(passwordInput),
			);
		});
