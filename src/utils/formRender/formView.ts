...
export interface FormView {
    ...
    updatePasswordVisibility: (showPassword: boolean, showPasswordConfirm: boolean) => void;
 ...
 */
export function createFormView(container: HTMLElement | null, template: HandlebarsTemplateDelegate): FormView {
    ...
    function cacheErrorElements(): ErrorCache | null {
        ...
        const errorContainer = form.querySelector('[data-error-container]') as HTMLElement | null;
        const errorText = errorContainer?.querySelector('[data-error-text]') as HTMLElement | null || errorContainer;
        const fields = new Map<string, { input: HTMLInputElement; error: HTMLElement | null }>();
        const inputs = form.querySelectorAll('input[name]');

        inputs.forEach((input) => {
            const element = input as HTMLInputElement;
            const name = element.name;
            const errorEl = form.querySelector(`[data-field-error="${name}"]`) as HTMLElement | null;
     ...
     */
    function renderErrors(errors: Record<string, string> = {}, serverError: string = ''): void {
    ...
    function updatePasswordVisibility(showPassword: boolean, showPasswordConfirm: boolean): void {
        ...
        if (passwordInput) {
            ...
            const passwordToggle = wrapper?.querySelector('[data-toggle-password]') as HTMLElement | null;
            if (passwordToggle) {
                const eyeOpen = passwordToggle.querySelector('.input__eyeIcon--open') as HTMLElement | null;
                const eyeClosed = passwordToggle.querySelector('.input__eyeIcon--closed') as HTMLElement | null;
        ...
        const confirmInput = container?.querySelector('input[name="passwordConfirm"]') as HTMLInputElement | null;
        if (confirmInput) {
            confirmInput.type = showPasswordConfirm ? 'text' : 'password';
            const wrapper = confirmInput.closest('.input__wrapper');
            const confirmToggle = wrapper?.querySelector('[data-toggle-password]') as HTMLElement | null;
            if (confirmToggle) {
                const eyeOpen = confirmToggle.querySelector('.input__eyeIcon--open') as HTMLElement | null;
                const eyeClosed = confirmToggle.querySelector('.input__eyeIcon--closed') as HTMLElement | null;
                if (eyeOpen && eyeClosed) {
