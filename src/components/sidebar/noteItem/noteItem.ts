...
export default class NoteItem extends Component {
    ...
    private cacheElements(): void {
        this.element = this.domElement?.querySelector('[data-name="noteItem"]') || null;
        this.titleElement = this.domElement?.querySelector('[data-name="noteItemTitle"]') || null;
        this.contentZone  = this.titleElement;
        this.addSubnoteBtn = this.domElement?.querySelector('[data-action="addSubnote"]') || null;
        this.settingsBtn = this.domElement?.querySelector('[data-action="settings"]') || null;
    }
    ...
    private attachEvents(): void {
        if (this.contentZone) {
            ...
            this.contentZone.addEventListener('click', this.boundHandlers.onContentClick);
        }

        if (this.titleElement) {
            this.boundHandlers.onTitleDoubleClick = (e: Event) => {
                e.stopPropagation();
                this.options.onTitleDoubleClick(this.options.note.ID, this.titleElement as HTMLElement);
            };
            this.titleElement.addEventListener('dblclick', this.boundHandlers.onTitleDoubleClick);
        }
        ...
        if (this.addSubnoteBtn) {
            ...
            this.addSubnoteBtn.addEventListener('click', this.boundHandlers.onAddSubnote);
        }

        if (this.settingsBtn) {
            this.boundHandlers.onSettings = (e: Event) => {
                e.stopPropagation();
                this.options.onSettingsClick(this.options.note.ID, this.settingsBtn as HTMLElement);
            };
    ...
    private detachEvents(): void {
        if (this.contentZone && this.boundHandlers.onContentClick) {
            this.contentZone.removeEventListener('click', this.boundHandlers.onContentClick);
        }
        if (this.titleElement && this.boundHandlers.onTitleDoubleClick) {
            this.titleElement.removeEventListener('dblclick', this.boundHandlers.onTitleDoubleClick);
        }
        if (this.addSubnoteBtn && this.boundHandlers.onAddSubnote) {
            this.addSubnoteBtn.removeEventListener('click', this.boundHandlers.onAddSubnote);
        }
        if (this.settingsBtn && this.boundHandlers.onSettings) {
            this.settingsBtn.removeEventListener('click', this.boundHandlers.onSettings);
        }
    }
