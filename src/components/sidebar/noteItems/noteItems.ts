...
export default class NoteItems extends Component {
    ...
    onRender(): void {
        const container = this.domElement?.querySelector('[data-notes-container="true"]') ;
    ...
    private renderNoteItems(): void {
        if (!this.container) {
            console.log('ВОТ ГДЕ ОШИБКА');
            return;
        }
        const { notes, activeNoteId, onNoteClick, onAddSubnote, onSettingsClick, onTitleDoubleClick } = this.options;
        notes.forEach((note) => {
            const placeholder = this.container?.querySelector(`[data-note-container="${note.ID}"]`);
    ...
    private addNoteItem(note: Note, isActive: boolean): void {
        ...
        const { onNoteClick, onAddSubnote, onSettingsClick, onTitleDoubleClick } = this.options;
        ...
        const nextContainer = nextNoteId ? this.container.querySelector(`[data-note-container="${nextNoteId}"]`) : null;
        if (nextContainer) {
    ...
    private reorderNotes(notes: Note[]): void {
        ...
        const correctOrder = notes.map(note => this.container?.querySelector(`[data-note-container="${note.ID}"]`));
