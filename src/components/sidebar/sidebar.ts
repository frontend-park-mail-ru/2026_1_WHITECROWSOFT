...
export class Sidebar {
    ...
    private _bindEvents(): void {
        ...
        const addBlockBtn = this.container.querySelector('#addSubnoteBtn') as HTMLButtonElement | null;
        if (addBlockBtn) {
        ...
        this.container.addEventListener('sidebar:openPopup', (e: Event) => {
            const customEvent = e as CustomEvent<{ noteId: string | number; anchor: HTMLElement }>;
        ...
        this.unsubscribeActiveNoteId = store.subscribe('activeNoteId', (noteId: string | number | null) => {
    ...
    private _openNotePopup(noteId: string | number, anchorElement: HTMLElement): void {
        this.currentPopup?.close();
    ...
    private _setActiveNote(noteId: string | number | null): void {
    ...
    private _updateNotes(notes: Note[]): void {
    ...
    private _updateUser(user: User | null): void {
    ...
    private _startInlineEdit(noteId: string | number): void {
        ...
        const noteItem = this.container.querySelector(`.sidebar__noteItem[data-note-id="${noteId}"]`) as HTMLElement | null;
        if (noteItem) {
            const noteItemTitle = noteItem.querySelector('.sidebar__noteItemTitle') as HTMLElement | null;
            if (noteItemTitle) {
