...
export function bindNavigationEvents(container: HTMLElement): void {
    container.addEventListener('click', (e: MouseEvent) => {
        ...
        const settingsBtn = target.closest('[data-action="settings"]') as HTMLElement | null;
        if (settingsBtn) {
            e.preventDefault();
            e.stopPropagation();
        ...
        const addSubnoteBtn = target.closest('[data-action="addSubnote"]') as HTMLElement | null;
        if (addSubnoteBtn) {
            e.preventDefault();
            e.stopPropagation();
            return;
        ...
        const createNoteBtn = target.closest('[data-action="newNote"]') as HTMLElement | null;
        if (createNoteBtn) {
            e.preventDefault();
            e.stopPropagation();
    ...
    container.addEventListener('dblclick', (e: MouseEvent) => {
        const target = e.target as HTMLElement;
        const noteItemTitle = target.closest('.sidebar__noteItemTitle') as HTMLElement | null;
        if (noteItemTitle) {
            e.preventDefault();
            e.stopPropagation();
            const noteItem = noteItemTitle.closest('.sidebar__noteItem') as HTMLElement | null;
            const noteId = noteItem?.dataset.noteId;
            if (noteId) {
...
export function startInlineEdit(noteItemTitle: HTMLElement, noteId: string | number): void {
...
export function updateActiveNote(container: HTMLElement, noteId: string | number | null): void {
